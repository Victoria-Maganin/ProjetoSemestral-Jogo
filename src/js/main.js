...
const jogador02 = document.querySelector('#El');
...
// Ele remove para que justamente ao vc colocar em outra li ele remova do antigo e joga para o novo
personagens.forEach((personagem) => {
    ...
    personagem.addEventListener('click', () => {
        ...
        switch (personagem.dataset.number) {
            ...
            case '05':
                var idCharacterAPI = 1009629;            //Foto para mudar no card
                break;
        ...
        fetch(``).then((responde) => {
            ...
            jsonParsed.data.results.forEach(element => {
                ...
                // Colocar descrição
                 description.innerHTML = element.description == '' ? `Sorry, we did not find any information about the character :( BUT if you wanna know more about <b>${element.name}</b> go check out the comics: ` : element.description;
            });
...
let btnConf = document.querySelector('#conf').addEventListener('click', () => {
    ...
    if (exportCharacterCont === 2) {
        ...
        if (typeof (Storage) === 'function') {
            ...
            alert('Infelizmente o seu navegador não suporta , por favor tente usar outro!')
        }
